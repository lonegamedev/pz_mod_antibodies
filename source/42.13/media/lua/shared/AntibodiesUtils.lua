...
local function lagrange(points, x)
	if #points > 0 then
		return tab_sum(tab_map(points, function(ref_point, index)
			local subset = tab_filter(points, function(_, i)
				return i ~= index
			end)
			local numenator = ref_point[2]
				* tab_product(tab_map(subset, function(point)
					return x - point[1]
				end))
			local denominator = tab_product(tab_map(subset, function(point)
				return ref_point[1] - point[1]
			end))
...
local format_change = function(f, chars)
	if f > 0 then
		return "+" .. format_float(f, chars)
	elseif f < 0 then
		return format_float(f, chars)
	end
	return format_float(f, chars)
...
local bool_to_number = function(value)
	return value and 1 or 0
end

local number_or_bool_to_number = function(value)
	if type(value) == "number" then
		return value
	else
		return bool_to_number(value)
	end
end

local number_or_zero = function(value)
	if value == nil then
		return 0
	end
	return value
...
function tableToJson(tbl)
	...
	local function encode(val)
		if type(val) == "table" then
			local items = {}
			for k, v in pairs(val) do
				table.insert(items, '"' .. escapeStr(tostring(k)) .. '":' .. encode(v))
			end
...
AntibodiesUtils.is_table_empty = is_table_empty
AntibodiesUtils.lerp = lerp
AntibodiesUtils.format_float = format_float
AntibodiesUtils.bool_to_number = bool_to_number
AntibodiesUtils.number_or_zero = number_or_zero
AntibodiesUtils.format_change = format_change
...
AntibodiesUtils.tableToJson = tableToJson
