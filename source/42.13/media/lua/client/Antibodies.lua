...
local function getKnoxInfectionLevel(character)
...
local function getKnoxInfectionStage(player)
	...
	if isInfected then
		...
		local infectionLevel = getKnoxInfectionLevel(player)
...
local function consumeKnoxInfection(player, medicalFile)
	...
	local newInfection = AntibodiesUtils.clamp(getKnoxInfectionLevel(player) - healStep, 0, 100)
